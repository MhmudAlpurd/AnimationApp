...
public class LottieFontViewGroup extends FrameLayout {
        ...
        LottieComposition.Factory.fromAssetFileName(getContext(), "Mobilo/BlinkingCursor.json",
                new OnCompositionLoadedListener() {
                    ...
                    public void onCompositionLoaded(LottieComposition composition) {
                        ...
                        cursorView.setScaleX(0.5f);
                        cursorView.setScaleY(0.5f);
                        addView(cursorView);
                    }
                });
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < views.size(); i++) {
            ...
            if (!fitsOnCurrentLine(currentX, view)) {
                ...
                currentY += view.getMeasuredHeight()/2;
            }
            currentX += 100;
        }

        setMeasuredDimension(getMeasuredWidth(),
                currentY/2 + views.get(views.size() - 1).getMeasuredHeight() * 2);
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        for (int i = 0; i < views.size(); i++) {
            ...
            view.layout(currentX, currentY/2, currentX + view.getMeasuredWidth(),
    ...
    private View createSpaceView() {
        ...
        spaceView.setScaleX(0.5f);
        spaceView.setScaleY(0.5f);
        return spaceView;
    }
}
